...
export async function POST(req: Request) {
  try {
    ...
    const prompt = `
      ...
      3. **유튜브 검색 키워드('keyword' 필드)에는 절대로 '#' 기호를 포함하지 마세요.** 특수기호 없이 자연어 키워드로만 검색해야 결과가 더 정확합니다.
      4. 다음 지표별 키워드 후보 중에서 상황에 가장 적합한 하나를 골라 'keyword'로 출력하세요:
      ...
      5. 상담 기법, 마케팅 전략, 고객 응대, 조직 관리 등 실무적인 테마를 지표와 연계하세요.
      ...
      다음 JSON 형식으로만 응답해 주세요:
      {
        ...
        "keyword": "유튜브 검색칸에 표시할 핵심 키워드 (예: 상담 화법)",
