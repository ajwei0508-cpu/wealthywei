...
export default function DonguibogamPage() {
  ...
  // --- Standardized Data Mapping for donguibogam ---
  ...
  const handleDeleteSelected = async () => {
    console.log(`[DonguibogamPage] Attempting to delete ${selectedMonthsForDelete.length} months`);
  ...
  const handleDeleteAll = async () => {
    ...
    console.log("[DonguibogamPage] Attempting to delete ALL data");
  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen bg-[#0A0E1A] text-white font-sans selection:bg-gold-500/30">
              ...
              <div className="flex items-center gap-2 relative">
                {pendingDelete?.type === "single" && pendingDelete.id === selectedMonth ? (
                  <div className="flex items-center gap-2 animate-in fade-in zoom-in duration-200">
                    <button
                      onClick={() => {
                        console.log(`[DonguibogamPage] Confirmed reset for ${selectedMonth}`);
                  ...
                  <button
                    onClick={() => { 
                      console.log(`[DonguibogamPage] Reset button clicked for ${selectedMonth}`);
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              {!isManageMode ? (
                <button 
                  onClick={() => {
                    console.log("[DonguibogamPage] Entering Data Management Mode");
        ...
        {/* Treatment Breakdown Section (Donguibogam Specific) */}
        <motion.div 
        ...
        >
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <p className="text-slate-500 text-xs mt-1 ml-13">항목별 진료 건수 및 비중 분석</p>
            </div>
            <div className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black text-amber-500 uppercase tracking-widest">
              Standard Format: Donguibogam
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(data.treatments || {}).length > 0 ? (
              Object.entries(data.treatments).sort((a: any, b: any) => b[1] - a[1]).map(([name, count]: any, idx: number) => (
                <div key={idx} className="p-5 rounded-2xl bg-white/5 border border-white/10 hover:border-amber-500/30 transition-all group">
                  <div className="flex justify-between items-start mb-3">
                    <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Rank {idx + 1}</span>
                    <span className="px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-500 text-[10px] font-bold">건수 {count}회</span>
                  </div>
                  <h3 className="text-white font-bold mb-2 group-hover:text-amber-400 transition-colors">{name}</h3>
                  <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-amber-500 rounded-full"
                      style={{ width: `${Math.min(100, (count / (data.totalPatients || 1)) * 100)}%` }}
                    />
                  </div>
                </div>
              ))
              ...
              <div className="col-span-full py-12 text-center text-slate-500 italic text-sm">
                진료 항목 데이터가 없습니다. 엑셀 파일을 업로드해 주세요.
              </div>
            )}
          </div>
        </motion.div>
