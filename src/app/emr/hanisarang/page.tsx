...
export default function HanisarangPage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<{ id: string; type: "single" | "bulk" | "all" } | null>(null);
  ...
  // --- Standardized Data Mapping for hanisarang ---
  ...
  const handleDeleteSelected = async () => {
    console.log(`[HanisarangPage] Attempting to delete ${selectedMonthsForDelete.length} months`);
    if (pendingDelete?.type !== "bulk") {
      setPendingDelete({ id: "bulk", type: "bulk" });
      return;
    }
  ...
  const handleDeleteAll = async () => {
    ...
    console.log("[HanisarangPage] Attempting to delete ALL data");
    if (pendingDelete?.type !== "all") {
      setPendingDelete({ id: "all", type: "all" });
      return;
    }
  ...
  useEffect(() => {
    async function getInsight() {
      if (data.totalRevenue > 0) {
        ...
        setInsight(""); // 초기화
        try {
          await generateClinicInsightStream(currentEntry, (text) => {
            setInsight(text);
          });
    ...
    getInsight();
  }, [data.totalRevenue, currentEntry, selectedMonth]);
  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen bg-[#0A0E1A] text-white font-sans selection:bg-emerald-500/30">
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-8">
            <button 
              onClick={() => router.push("/")} 
              className="text-slate-500 hover:text-emerald-400 flex items-center gap-2 text-sm transition-all group"
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-[#10B981] hover:bg-[#059669] text-[#0A0E1A] px-6 py-3 rounded-2xl text-xs font-black transition-all shadow-[0_0_20px_rgba(251,191,36,0.3)] active:scale-95 group"
              >
                <Upload size={18} className="group-hover:bounce-y transition-transform" /> 
                <span className="tracking-tight">신규 데이터 업로드</span>
              </button>
              <div className="flex items-center gap-2 relative">
                {pendingDelete?.type === "single" && pendingDelete.id === selectedMonth ? (
                  <div className="flex items-center gap-2 animate-in fade-in zoom-in duration-200">
                    <button
                      onClick={() => {
                        console.log(`[HanisarangPage] Confirmed reset for ${selectedMonth}`);
                        deleteMonthlyData(selectedMonth);
                        setPendingDelete(null);
                      }}
                      className="bg-rose-500 hover:bg-rose-600 text-white px-3 py-2 rounded-xl text-[10px] font-black uppercase transition-all shadow-lg shadow-rose-500/20"
                    >
                      진짜 삭제
                    </button>
                    <button
                  ...
                  <button
                    onClick={() => { 
                      console.log(`[HanisarangPage] Reset button clicked for ${selectedMonth}`);
                      setPendingDelete({ id: selectedMonth, type: "single" });
                    }}
                    className="group p-2.5 rounded-2xl bg-white/5 border border-white/10 text-slate-500 transition-all hover:bg-red-500/10 hover:text-red-400 relative z-50 cursor-pointer"
                    title="데이터 초기화"
                  >
                    <Trash2 size={18} />
                  </button>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs font-bold tracking-wider uppercase">
                  EMR Intelligence
                </span>
              ...
              <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight leading-tight">
                한의사랑 <span className="text-slate-400">경영 리포트</span>
              </h1>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              ...
              {uniqueYears.map(year => (
                <button
                  ...
                  className={`px-5 py-2 rounded-xl text-xs font-bold transition-all border shrink-0 ${
                    displayYear === year 
                    ? "bg-white/10 border-emerald-500/50 text-emerald-400 shadow-lg shadow-emerald-500/5" 
            ...
            <div className="flex items-center gap-2">
              {!isManageMode ? (
                <button 
                  onClick={() => {
                    console.log("[HanisarangPage] Entering Data Management Mode");
                    setIsManageMode(true);
                  }}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-slate-400 text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all hover:text-white group relative z-50 cursor-pointer"
                >
                ...
                <div className="flex items-center gap-2 relative z-50">
                  <button 
                    ...
                    className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-slate-300 text-[10px] font-black uppercase hover:bg-white/10 transition-all cursor-pointer"
                  >
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer font-black text-[10px] uppercase ${
                      pendingDelete?.type === "bulk" 
                        ? "bg-rose-600 text-white ring-2 ring-white/20 scale-105" 
                        : "bg-rose-500 text-white hover:bg-rose-600"
                    }`}
                  >
                    <Trash2 size={14} /> {pendingDelete?.type === "bulk" ? "정말 삭제할까요?" : `${selectedMonthsForDelete.length}개 삭제`}
                  </button>
                  <button 
                    onClick={handleDeleteAll}
                    className={`px-4 py-2 rounded-xl font-black text-[10px] uppercase border transition-all cursor-pointer ${
                      pendingDelete?.type === "all"
                        ? "bg-rose-700 text-white border-white/50 scale-105"
                        : "bg-slate-800 text-rose-400 border-rose-500/30 hover:bg-rose-900/20"
                    }`}
                  >
                    {pendingDelete?.type === "all" ? "전체 데이터 삭제 확정" : "초기화"}
                  </button>
                  {pendingDelete && (
                    <button 
                      onClick={() => setPendingDelete(null)}
                      className="px-4 py-2 rounded-xl bg-white/20 text-white text-[10px] font-black uppercase hover:bg-white/30 transition-all cursor-pointer"
                    >
                      취소
                    </button>
                  )}
                  <button 
                    ...
                    className="px-4 py-2 rounded-xl bg-white/10 text-white text-[10px] font-black uppercase hover:bg-white/20 transition-all cursor-pointer"
                  >
                    닫기
                  </button>
                </div>
              ...
              {filteredMonths.map((m) => (
                <button
                  ...
                  className={`flex-1 min-w-[70px] py-3 rounded-xl border transition-all flex flex-col items-center justify-center relative ${
                    ...
                    ? "bg-[#10B981] border-[#059669] text-[#0A0E1A] shadow-lg shadow-amber-500/20" 
          ...
          <div className="relative group overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-[#1A1F35] to-[#0D1117] border border-emerald-500/30 shadow-2xl p-10 mb-8">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
              <div>
                <p className="text-emerald-500/80 text-xs font-black uppercase tracking-[0.2em] mb-3">Target Month Performance</p>
                <h2 className="text-white text-xl font-medium mb-1">
                  <span className="text-emerald-400 font-black">{formatMonth(selectedMonth)}</span> 총 진료비 매출액
                </h2>
                <p className="text-slate-500 text-sm">한의사랑 시스템에서 집계된 전체 발생 매출입니다.</p>
              </div>
            ...
            <div className="h-48 flex items-end justify-between gap-1 px-2">
              {(() => {
                ...
                return finalTrendMonths.map((m) => {
                  const mData = monthlyData[m]?.hanisarangData;
                  const rev = mData?.totalRevenue || 0;
                  const maxRev = Math.max(...finalTrendMonths.map(mm => monthlyData[mm]?.hanisarangData?.totalRevenue || 1));
                  ...
                  return (
                    ...
                    >
                      <div className="w-full bg-white/5 rounded-t-lg relative overflow-hidden h-40 flex items-end">
                        <div 
                          className={`w-full transition-all duration-700 ease-out ${selectedMonth === m ? "bg-[#10B981] shadow-[0_0_20px_rgba(251,191,36,0.6)]" : "bg-white/10 group-hover:bg-white/20"}`}
                          style={{ height: `${height}%` }}
                        />
                        <div 
                      ...
                      <span className={`text-[9px] font-black transition-colors whitespace-nowrap ${selectedMonth === m ? "text-[#10B981]" : "text-slate-600"}`}>
                        {m.split("-")[1]}월
                      </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <div className="bg-[#111624] rounded-[2rem] p-6 border border-white/5 hover:border-emerald-500/30 transition-all group shadow-xl">
              <div className="flex justify-between items-start mb-6">
                <div className="p-3 bg-emerald-500/10 rounded-2xl text-emerald-500 group-hover:bg-emerald-500 group-hover:text-[#0A0E1A] transition-all">
                  <TrendingUp size={22} />
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-end">
                  <span className="text-[10px] text-emerald-500 font-bold uppercase">{formatMonth(selectedMonth)}</span>
          ...
          <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 flex flex-wrap items-center justify-center gap-x-12 gap-y-4 border border-white/10">
            <div className="flex flex-col items-end">
              ...
              <div className="flex items-baseline gap-2">
                <p className="text-2xl font-black text-emerald-500 leading-none">{formatNumber(data.totalPatients)}<span className="text-xs font-bold ml-1 text-slate-500">명</span></p>
                {compareMonth && (
            ...
            <div className="space-y-6">
              ...
              <div className="p-5 rounded-2xl bg-white/5 border border-white/10 group hover:bg-white/10 transition-all duration-300">
                ...
                <div className="flex justify-between mt-2">
                  ...
                  <span className="text-[10px] text-emerald-400 font-bold">{((data.totalReceived / (data.patientTotal || 1)) * 100).toFixed(1)}%</span>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 flex items-start gap-3">
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  결산 데이터상 <span className="text-emerald-400 font-bold">{formatNumber(data.receivables + data.discountTotal)}원</span>의 경영 누수(미수+할인)가 발생했습니다. 차트 내 수납 상태가 '미수'인 환자 리스트를 확인해 주세요.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-center p-8 mb-4 relative min-h-[220px]">
              ...
              <div className="flex flex-col gap-6 w-full">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="h-3 grow bg-slate-800 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-emerald-500 rounded-full shadow-[0_0_10px_rgba(234,179,8,0.5)]" 
...
function RollingNumber({ value }: { value: number }) {
  ...
  return (
    <span className="text-5xl md:text-7xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-white to-emerald-400 animate-gradient drop-shadow-[0_0_15px_rgba(251,191,36,0.3)]">
