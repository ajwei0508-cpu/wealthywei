        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <header className="flex items-end justify-between">
            <div>
              ...
              <p className="text-white/50 mt-2 font-medium leading-relaxed">
                직원 교육용 계정을 관리합니다.<br/>
          ...
          <div className="bg-white/5 p-8 rounded-[24px] shadow-xl border border-white/5 animate-in fade-in slide-in-from-top-4">
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              ...
              <button 
                ...
                disabled={isGenerating}
                className="px-6 py-3 bg-indigo-500/10 text-indigo-400 hover:bg-indigo-100 rounded-xl font-bold transition-colors flex items-center gap-2 border border-indigo-200"
              >
            ...
            {inviteCodes.length > 0 && (
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-white/50 uppercase">활성화된 초대 코드</h4>
                {inviteCodes.map(code => {
                  ...
                  return (
                    <div key={code.code} className="flex items-center justify-between p-4 rounded-xl border border-white/10 bg-white/5">
                      <div className="flex items-center gap-4">
                        <span className="text-2xl font-black text-white/90 tracking-[0.2em]">{code.code}</span>
                        <div className="flex flex-col">
                          <span className={`text-xs font-bold ${isExpiringSoon ? 'text-rose-400' : 'text-white/50'}`}>
                      ...
                      <div className="flex items-center gap-2">
                        <button 
                          ...
                          className={`p-2 rounded-lg transition-colors ${isCopied ? 'bg-emerald-100 text-emerald-400' : 'bg-white/5 border border-white/10 text-white/70 hover:bg-white/10'}`}
                        >
                          {isCopied ? <CheckCircle2 size={18} /> : <Copy size={18} />}
                        </button>
                        <button 
                          onClick={() => handleDeleteCode(code.code)}
                          className="p-2 bg-white/5 border border-white/10 text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors"
                          title="코드 즉시 폐기"
                        >
                          <Trash2 size={18} />
                        </button>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-white/5">
                    <tr>
                      <th className="px-6 py-4 text-xs font-bold text-white/50 uppercase">소속 한의원</th>
                      <th className="px-6 py-4 text-xs font-bold text-white/50 uppercase">이름</th>
                      <th className="px-6 py-4 text-xs font-bold text-white/50 uppercase">휴대폰 번호 (아이디)</th>
                      <th className="px-6 py-4 text-xs font-bold text-white/50 uppercase">가입일</th>
                      <th className="px-6 py-4 text-xs font-bold text-white/50 uppercase text-center">교육 이수율</th>
                      <th className="px-6 py-4 text-xs font-bold text-white/50 uppercase text-right">관리</th>
                    </tr>
                  </thead>
                      ...
                      return (
                        <tr key={s.id} className="hover:bg-white/5/50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            ...
                            <div className="text-[10px] text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded mt-1 inline-block font-bold">교육 전용</div>
                          </td>
                          <td className="px-6 py-4 font-medium text-white/80">{s.phone}</td>
                          <td className="px-6 py-4 text-sm text-white/50">{new Date(s.created_at).toLocaleDateString('ko-KR')}</td>
                          <td className="px-6 py-4 text-center">
                            <div className="flex flex-col items-center gap-1.5 w-24 mx-auto">
                              ...
                              <div className="flex items-center gap-1.5">
                                <span className="text-[10px] font-black text-white/70">{progressPct}%</span>
