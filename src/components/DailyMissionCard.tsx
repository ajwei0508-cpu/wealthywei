...
export const DailyMissionCard = ({ data, userName, emrType }: DailyMissionCardProps) => {
  ...
  const mission = useMemo(() => {
    ...
    const patientMetrics = data?.patientMetrics || { new: 0, total: 0, auto: 0, dailyAvg: 0 };
    const leakage = data?.leakage || { receivables: 0, discountTotal: 0, roundOffTotal: 0 };
    const generatedRevenue = data?.generatedRevenue || { total: 0, nonCovered: 0 };
    ...
    const missions = [
      ...
      {
        title: "미수금 점검",
        description: `현재 미수금이 ${new Intl.NumberFormat("ko-KR").format(leakage.receivables)}원입니다. 가장 오래된 미수 환자 1명에게 확인 전화를 해볼까요?`,
      ...
      {
        title: "진료 효율성 체크",
        description: `일평균 환자 수가 ${patientMetrics.dailyAvg?.toFixed(1) || "0.0"}명입니다. 오늘 점심시간 5분만 할애하여 대기 시간을 줄일 수 있는 동선을 점검해보세요.`,
      ...
      {
        title: "비급여 상담 매뉴얼",
        description: `비급여 비중이 현재 ${generatedRevenue.total > 0 ? ((generatedRevenue.nonCovered / generatedRevenue.total) * 100).toFixed(1) : 0}%입니다. 오늘 가장 많이 나가는 비급여 항목의 상담 멘트 하나만 고쳐보세요.`,
